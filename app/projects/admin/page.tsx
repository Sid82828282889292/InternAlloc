...
// import CreateProjectStepper from "@/components/admin/CreateProjectStepper";
...
//     console.log("✅ Loaded Requirements:", reqData);
...
//       .map((r) => {
//         const skill = skills.find((s) => s.uuid?.trim() === r.skill_id?.trim());
//         if (!skill) {
//           console.warn(`❌ Skill not found for skill_id: ${r.skill_id}`);
//         }
//         return skill ? skill.name : `Unknown Skill (${r.skill_id})`;
//       });
...
//                 >
//                   <GlassCard className="p-6 bg-white/50 dark:bg-gray-800/40 backdrop-blur-sm shadow-md transition-all duration-300 hover:shadow-xl hover:ring-2 hover:ring-pink-500 hover:ring-offset-2 hover:ring-offset-white dark:hover:ring-offset-gray-900">
...
export default function AdminProjectsPage() {
  ...
  const getProjectSkills = (projectId: string) =>
    requirements
      .filter((r) => r.project_id.trim() === projectId.trim())
      .sort((a, b) => a.priority - b.priority)
      .map((r) => {
        const skill = skills.find((s) => s.id.trim() === r.skill_id.trim());
