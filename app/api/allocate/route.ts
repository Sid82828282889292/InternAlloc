  ...
  try {
    ...
    for (const project of unassignedProjects) {
      ...
      if (!projectSkills?.length) {
        allocationResults.push({ project: project.name, message: 'No required skills listed.' });
        continue;
      }
      ...
      for (const intern of interns) {
        ...
        score -= (intern.current_hours / intern.goal_hours) * 5;
      ...
      if (!bestMatch) {
        allocationResults.push({ project: project.name, message: 'Not enough hours' });
        continue;
      }

      if (bestScore <= 0) {
        allocationResults.push({ project: project.name, message: 'No suitable candidate' });
        continue;
      }
      ...
      allocationResults.push({ project: project.name, message: `Assigned to ${bestMatch.email}` });
    }

    return NextResponse.json({ success: true, results: allocationResults });
  } catch (e) {
    console.error('AI Allocation Error:', e);
    return NextResponse.json({ error: 'Server error' }, { status: 500 });
  }
